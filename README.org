...
sudo pacman -S python python-pip pyright
   #+end_src
   - if you installed anaconda, add it to emacs PATH (It's better ro write the full path without ~~/~):
      #+begin_src emacs-lisp
...
fish_add_path $HOME/.ghcup/bin
   #+end_src
   + Add ghcup to doom emacs PATH (It's better ro write the full path without ~~/~):
   #+begin_src emacs-lisp
