...
StartupWMClass=Emacs
     #+end_src
4. Alias ~/usr/bin/eamcsclient -c -a 'emacs' ~ to ~emacs~. this will help launch doom emacs client through terminal. for example in fish shell:
   #+begin_src shell
...
sudo echo "/usr/bin/emacs --daemon" >> /etc/profile.d/emacsclient.sh
